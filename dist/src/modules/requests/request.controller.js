...
export class RequestController {
    ...
    static async transition(req, res) {
        ...
        const updated = await RequestService.transitionRequest(id, req.user.userId, req.user.role, status);
        res.json(updated);
    }
    ...
    static async getAccepted(req, res) {
        try {
            const requests = await RequestService.getAcceptedRequests(req.user.userId, req.user.role);
            res.json(requests);
        }
        catch (error) {
    ...
    static async getClosed(req, res) {
        try {
            const requests = await RequestService.getClosedRequests(req.user.userId, req.user.role);
            res.json(requests);
        }
        catch (error) {
            res.status(500).json({ message: error.message });
    ...
    static async cancel(req, res) {
        try {
            ...
            const updated = await RequestService.transitionRequest(id, userId, "USER", "CANCELLED");
